...
class Editor {
    ...
    streamParser = {
        ...
        token: (stream, state) => {
            if ( stream.sol() ) {
                if ( stream.match(/^---\s*$/) ) { return 'AdReaper-boundary'; }
                if ( stream.match(/^# ---\s*$/) ) { return 'AdReaper-boundary AdReaper-comment'; }
                if ( stream.match(/\.\.\.\s*$/) ) { return 'AdReaper-boundary'; }
            }
            const c = stream.peek();
            if ( c === '#' ) {
            ...
            if ( scope === 0 && stream.match(/^[^:]+(?=:)/) ) {
                state.scope = 1;
                return 'AdReaper-keyword';
            }
            if ( scope === 1 && stream.match(/^:(?: |$)/) ) {
                return 'AdReaper-punctuation';
            }
            if ( stream.match(/^- /) ) {
                return 'AdReaper-punctuation';
            }
