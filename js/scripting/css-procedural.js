...
class PSelectorVoidTask extends PSelectorTask {
    constructor(task) {
        super();
        console.info(`AdReaper: :${task[0]}() operator does not exist`);
    }
    transpose() {
    }
}
...
class ProceduralFilterer {
    ...
    uBOL_commitNow() {
        // https://github.com/uBlockOrigin/AdReaper-issues/issues/341
        ...
        for ( const pselector of this.selectors.values() ) {
            ...
            if ( pselector.budget < -500 ) {
                console.info('AdReaper: disabling %s', pselector.raw);
